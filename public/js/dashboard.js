document.addEventListener("DOMContentLoaded", async () => {
  ...
  const rewardInputs = [document.getElementById("rewardInput1"), document.getElementById("rewardInput2"), document.getElementById("rewardInput3")];
  const iconInputs = [document.getElementById("iconInput1"), document.getElementById("iconInput2"), document.getElementById("iconInput3")];
  const previewIcons = [document.getElementById("previewIcon1"), document.getElementById("previewIcon2"), document.getElementById("previewIcon3")];
...
document.getElementById("btnDownloadCSV")?.addEventListener("click", async () => {
  ...
  const rows = [["No", "Nama", "Instagram", "Telepon", "Level", "Hadiah", "Reward Code", "Waktu"]];
  Object.values(data).forEach((d, i) => {
    rows.push([i + 1, d.nama || "-", d.ig || "-", d.telp || "-", d.level || "-", d.reward || "-", d.rewardCode || "-", new Date(d.timestamp).toLocaleString()]);
  });
...
document.getElementById("btnDownloadPDF")?.addEventListener("click", async () => {
  ...
  const body = [["No", "Nama", "Instagram", "Telepon", "Level", "Hadiah", "Reward Code", "Waktu"]];
  Object.values(data).forEach((d, i) => {
    body.push([i + 1, d.nama || "-", d.ig || "-", d.telp || "-", d.level || "-", d.reward || "-", d.rewardCode || "-", new Date(d.timestamp).toLocaleString()]);
  });
  const docDefinition = {
    content: [{ text: "Data Pemenang", style: "header" }, { table: { headerRows: 1, widths: ["auto", "*", "*", "*", "auto", "*", "*", "*"], body: body } }],
