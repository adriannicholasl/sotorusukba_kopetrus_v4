...
document.getElementById("btnDownloadCSV")?.addEventListener("click", async () => {
  ...
  Object.values(data).forEach((d, i) => {
    rows.push([i + 1, d.nama || "-", d.ig || "-", d.telp || "-", d.level || "-", d.reward || "-", new Date(d.timestamp).toLocaleString()]);
  });
...
document.getElementById("btnDownloadPDF")?.addEventListener("click", async () => {
  ...
  Object.values(data).forEach((d, i) => {
    body.push([i + 1, d.nama || "-", d.ig || "-", d.telp || "-", d.level || "-", d.reward || "-", new Date(d.timestamp).toLocaleString()]);
  });
